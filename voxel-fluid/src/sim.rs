...
/// Check if a cell has a downward outlet (air below or diagonal-down air neighbor).
/// When true, gravity/slope flow handles this cell — suppress horizontal spread.
#[inline]
    ...
    for &z in &order {
        for y in 0..size {
            for &x in &order {
                ...
                // Skip horizontal spread for cells with a downward outlet (slopes/ledges)
    ...
    for z in 0..size {
        for y in 0..size {
            for x in 0..size {
                ...
                // Horizontal spread — suppressed when downward outlet exists
                if new_cells[idx].level > MIN_LEVEL
