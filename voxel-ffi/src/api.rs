...
fn rust_pos_to_ue(p: glam::Vec3, world_scale: f32) -> FfiVec3 {
    FfiVec3 {
        x: p.x * world_scale,
        y: -p.z * world_scale,
        z: p.y * world_scale,
...
pub unsafe extern "C" fn voxel_request_list_top_pois(
    ...
    // Pull a generous candidate set from the tracker — we'll re-sort
    // alongside bridges and truncate to whatever the caller asked for.
    // Tracker map is bounded by TTL; size in normal play is hundreds.
    const CANDIDATE_LIMIT: usize = 64;
    let mut candidates: Vec<crate::poi_scanner::Poi> =
