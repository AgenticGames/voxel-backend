(function () {
    ...
    // original call sites below stay untouched: same paths, same bodies,
    // same JSON coming back, same 400 + plain-text error behaviour.
    var caveWorker = new Worker("worker.js");
    var pendingCalls = {};
    var nextCallId = 1;

    caveWorker.onmessage = function (e) {
        var msg = e.data;
        ...
        pending.resolve({
            ...
            status: msg.status,
            json: function () {
                try { return Promise.resolve(JSON.parse(msg.text)); }
                catch (err) { return Promise.reject(err); }
            },
            text: function () { return Promise.resolve(msg.text); }
        });
    };

    caveWorker.onerror = function (e) {
        // Fail every in-flight call rather than hanging the UI forever.
        Object.keys(pendingCalls).forEach(function (id) {
            pendingCalls[id].reject(new Error("Cave worker error: " + (e.message || "unknown")));
            ...
            if (mode === "water-place" || mode === "lava-carve") {
                ...
                .then(function (mineData) {
                    ...
                    if (!isLava) {
                        ...
                        apiFetch(apiUrl("/api/place-water"), {
                            method: "POST",
                            headers: { "Content-Type": "application/json" },
                            body: waterBody
                        });
    ...
    async function loadReport() {
        ...
        try {
            var resp = await apiFetch(apiUrl("/api/report"));
    ...
    async function loadObj(seed, btn) {
        ...
        try {
            var resp = await apiFetch(apiUrl("/api/obj/" + seed));
    ...
    async function loadObjFile(filename, btn) {
        ...
        try {
            var resp = await apiFetch(apiUrl("/api/obj-file/" + encodeURIComponent(filename)));
    ...
    async function loadCustomExports() {
        try {
            var resp = await apiFetch(apiUrl("/api/obj-files"));
            if (!resp.ok) return;
    ...
    genBtn.addEventListener("click", async function () {
        ...
        try {
            ...
            var body = parts.join("&");
            var resp = await apiFetch(apiUrl("/api/generate"), {
                method: "POST",
    ...
    if (runBatchBtn) {
        runBatchBtn.addEventListener("click", async function () {
            ...
            try {
                var resp = await apiFetch(apiUrl("/api/run-batch"), { method: "POST" });
    ...
    var GEOLOGICAL_FACTS = [
        "Simulating geological materials with realistic depth layering",
    ...
    async function preGenerateShowcase() {
        ...
        try {
            ...
            var parts = [
                ...
                // No closed=1: cave entrances stay open at the region faces.
                // Mining past the edge lazily grows the world a few chunks
                // before hitting sealed bedrock, so no see-through holes.
                "cavern_freq=0.002",
